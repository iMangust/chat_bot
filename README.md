# 🐾 TamaBot v1.5.3 — развлекательный Telegram-бот (тамагочи + достижения + топы)
...
группа обсуждения»**. Доступ к боту — **только в личных сообщениях и только для
подписчиков канала**: в группах/каналах бот молчит и ничего не пишет, взаимодействие
без подписки заблокировано. Управление — через интерактивные кнопки с единой
пагинированной навигацией. Запуск и управление — на **Windows Server** через BAT-скрипты
...
## 🧭 Навигация и доступ (v1.5.3)

**Доступ.** Бот отвечает **только в личных сообщениях**: любые сообщения и кнопки в
группах/каналах игнорируются молча, бот ничего туда не пишет (активность трекается
пассивно). Пользователи **без подписки на канал** (`CHANNEL_USERNAME`) не могут
взаимодействовать с ботом — вместо ответа показывается заглушка «🔒 Бот доступен только
подписчикам канала» с кнопками «📢 Подписаться» и «✅ Я подписался — проверить».
Проверка подписки кэшируется на 5 минут; при недоступном канале/сбое API действует
fail-open. Реализация: `middlewares/gate.py` (outer-middleware на все апдейты).

**Единая стилистика всех экранов:**
* контент — по **2 кнопки в ряд**, максимум **3 ряда** на страницу;
* 4-й ряд — навигация: `◀️` · `«Название 📖 i/n»` · `▶️` (перехлёст зациклен);
* выход с экрана — **одна** кнопка «🏠 Меню» внизу (без дублей «⬅️ Назад»).

**Главное меню** — две страницы: «🎮 Игра» (🐾 Питомец, 🛒 Магазин, 🧢 Мерч канала —
при `MERCH_ENABLED=true`) и «👤 Профиль» (📊 Статистика, 🏆 Награды, 🏅 Топы,
🖼 Карточка, ⚙️ Уведомления); плюс url-кнопка «🤝 Пригласить друга (+N 🪙)».
Хаб питомца — три страницы: «🧴 Уход», «🎒 Вещи», «🎮 Досуг» (игры, прогулка,
друзья, арена). Тот же стандарт у магазина, мерча, ачивок, топов и настроек.

---

## 🧩 Возможности (реализовано, v1.0–v1.5)

| Блок | Что внутри |
|---|---|
| 👋 Приветствие | Онбординг в ЛС, приветствие новичков канала (`chat_member` + фоновый скан), выбор имени/вида питомца; доступ — только подписчикам канала |
...
| 🧢 Мерч | Витрина мерча канала из главного меню (категории, карточки, журнал заявок), отключается `MERCH_ENABLED=false` |
| 🛡 Инфраструктура | Long polling, gate-middleware (только ЛС + проверка подписки на канал), rate-limit middleware, graceful shutdown, APScheduler (decay/очередь/стрики/отчёты/топ недели), loguru с ротацией, опциональный Redis/Memurai с in-memory fallback |
...
└── bot/
    ...
    │   ├── handlers/                     # start, welcome, tamagotchi, games, shop, merch,
    ...
    │   ├── middlewares/                  # gate (только ЛС + подписка), throttle, user, session
    │   ├── console/                      # TamaConsole (Textual): логи/дашборд/БД/настройки
    │   └── tasks/scheduler.py            # APScheduler-джобы
    ├── tests/                            # pytest: 167 passed (SQLite in-memory)
...
python -m pytest tests -q      # ожидаем: 167 passed
```
