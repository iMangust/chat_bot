...
> ⚠️ BAT-файлы сохранены в кодировке **cp866 (OEM Russian) с CRLF** — это обязательное
> требование cmd.exe. При редактировании сохраняйте их именно в cp866/OEM-866
> (Notepad++: Кодировки → Кириллица → OEM 866), иначе вместо русских строк cmd
> начнёт «выполнять» мусор (`'ый' is not recognized...`). Корректность форматов
> контролируется тестами `tests/test_bat_files.py`.
